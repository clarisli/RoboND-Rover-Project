...
Now the camera is at `(x, y) = (160, 160)` in the top-down thresholded images. The goal here was to convert it to a coordinate system that is fixed with respect to the robot - the camera is at `(x, y) = (0, 0)`.
...
In addition to convert it to the rover-centric coordinates system, I also swapped x and y axes so that they are consistent with the world coordinate system:
![alt text][image9]
...
I used a rotation matrix in the rotate step

![alt text][image12]

I did the rotation in the function `rotate_pix()` in cell 7 of the notebook.

Then in the translation step, I added the x and y components of the rover's position to the rotated x and y values calculated above.

A scale factor of 10 was used in this calculation, because in the rover space each pixel represents 0.1 x 0.1 m, and in the world map each pixel represents 1 x 1 m. 

I did the translation in the function `translate_pix()` in cell 7 of the notebook.
...
After converting the navigable terrain, obstacles, and rock pixels to the world space. I then Add each to different RGB channels. I assigned obstacle to the R channel, navigable to the B channel, and rock to the G channel.

I did this in the `process_image()` function in the notebook. 

##### Result

I applied the `process_image()` on test data and created a video [here](./output/test_mapping.mp4).
...
* Show the thresholded navigable terrain, obstacles, and rock on the left side of screen. I assigned them to the rover's vision image, showed navigable terrain in blue, bstacles in red, and rock in green.
* A near vision rover - I clipped out navigable terrain and obstacles that are out a certain range, because they tend to be more inaccurate. In this way I increased the fidelity.
* Only update the worldmap when the rover is flat, i.e., roll and pitch are near zero, because the perspective transform is only valid under this condition.

I did this in the function `perception_step()` in `perception.py`.

##### Decision Step

In order to increase the % mapped and find more rocks - the rocks always appear near the walls - I made the rover a "wall crawler" that always keeping a wall on its left. By removing navigable angles pointing to the right, the rover moves along the left wall all the way around the worldmap. 

I did this in line 45 of `decision.py`.

If the rover stopped moving for a while, it might be stucked. I did this in a function called `is_stuck()` in lines 6 to 17 of `decision.py`. 

If it gets stucked, the rover will be switched to the 'stuck' mode, and try to make turns or move backward to get out. I did this in the function `unstuck()` in lines 19 to 28 of `decision.py`.

If there's a rock, the rover will move toward it and adjust its speed according to the distance and it will try to stop and pick it up. I did this in lines 56 to 71 in `decision.py`.
...
In addition to the perception and decision steps, I've also increased the maximum velocity(max_vel) and acceleration(throttle_set) and brake setting(brake_set) to minimize total time.

I might improve the the project in following areas:

* The rover could get stucked in turning large circles around open-field. If the rover keeps turning in the same direction, I know it's looping and I can break it by making it turning in the opposite direction instead.
* Avoid rivisit the explored region.
